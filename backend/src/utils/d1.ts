...
export class D1Helper {
  ...
  async createEvent(event: Omit<Event, 'createdAt'>): Promise<void> {
    await this.run(
      `INSERT INTO events (id, title, category, event_type, aspect_ratio, price, duration, scene, camera, generation, status, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, unixepoch())`,
      event.id,
      event.title,
      event.category,
  ...
  async createGeneration(gen: Omit<Generation, 'createdAt' | 'completedAt'>): Promise<void> {
    await this.run(
      `INSERT INTO generations (id, user_id, event_id, input_image, output_image, output_video, agnes_job_id, status, error_message, captions, football, basketball, retry_image, retry_video, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, unixepoch())`,
      gen.id,
      gen.userId,
      gen.eventId,
  ...
  private mapEvent(row: Record<string, unknown>): Event {
    return {
      ...
      duration: (row.duration as number) ?? 5,
  ...
  private mapGeneration(row: Record<string, unknown>): Generation {
    return {
      ...
      basketball: (row.basketball as string) ?? undefined,
